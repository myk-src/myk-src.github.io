<template>
    <div class="resume" v-if="resumes && resumes.length > 0">
        ...
        <span v-for="(major, index) in resumes[0].education.major" :key="index">
            ...
            <h4>
                <span v-if="resumes[0].education.expected_grad_date > new Date().toISOString()">
                    Expected {{ new Date(resumes[0].education.expected_grad_date + -1).toLocaleDateString('en-US', { year: 'numeric', month: 'long' }) }}
                </span>
                <span v-else>
                    {{ new Date(resumes[0].education.expected_grad_date + -1).toLocaleDateString('en-US', { year: 'numeric', month: 'long' }) }}
                </span>
            ...
            <ul>
                ...
                <li>
                    ...
                    <ul class="courses">
                        <li v-for="course in resumes[0].education.courses" :key="course">{{ course }}</li>
                    </ul>
                </li>
