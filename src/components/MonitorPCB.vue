    ...
    <div v-else class="monitor-front">
      <div class="monitor-body">
        ...
        <div class="monitor-screen" :class="{ 'sw-wallpaper': isSoftwareMode }">
          ...
          <!-- THE SWITCH: Floats at the top of the screen -->
          ...
          <!-- CROSSFADE INNER CONTENTS -->
          <transition name="fade" mode="out-in">
            
            <!-- TERMINAL VIEW (SW MODE) -->
            <div class="terminal-wrapper" v-if="isSoftwareMode">
              <TerminalView />
            </div>

            <!-- PCB VIEW (HW MODE) -->
            <div class="physical-pcb" v-else>
...
/* Ensure the terminal doesn't overlap the floating switch */
.terminal-wrapper {
  height: 100%;
  width: 100%;
  padding: 1em;
...
.monitor-assembly.software-mode { z-index: 9000 !important; }
.software-mode .monitor-front { width: 100vw; }
.software-mode .monitor-body { padding: 0; border-radius: 6px; border: none; box-shadow: none; background-color: transparent;}
.software-mode .monitor-screen { height: 100dvh; border-radius: 6px; box-shadow: none; }
...
.monitor-body { width: 100%; background-color: #1a1a1a; padding: 15px 15px 30px 15px; border-radius: 4px; border-top: 1px solid #333; border-left: 1px solid #222; border-bottom: 4px solid #050505; border-right: 4px solid #050505; box-shadow: 15px 15px 30px rgba(0,0,0,0.8); position: relative; z-index: 2; transition: all 0.6s cubic-bezier(0.34, 1.56, 0.64, 1); }
...
.monitor-screen { background-image: url('@/assets/wallpaper/ascii.jpg'); background-size: cover; background-position: right bottom; background-repeat: no-repeat; background-color: var(--background-color); border-radius: 2px; overflow: hidden; position: relative; box-shadow: inset 0 0 10px #000; display: flex; flex-direction: column; transition: all 0.6s cubic-bezier(0.34, 1.56, 0.64, 1); }
...
/* The PCB Content */
.physical-pcb { margin: 1em; margin-top: 0; background-color: #141b18; background-image: radial-gradient(#2a3b33 1px, transparent 1px); background-size: 20px 20px; padding: 30px; height: 100%; overflow-y: auto; position: relative; border-radius: 1em; border: 1px solid #efefef; }
